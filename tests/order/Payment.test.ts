...
import AlterPayment from "../../src/application/usecase/invoice/AlterPayment"
...
import PayInvoice from "../../src/application/usecase/invoice/PayInvoice"
...
import { PAYMENT_STATUS } from "../../src/domain/entities/Payment"
...
describe("Teste de pagamentos de comanda", ()=>{
    ...
    it("deve iniciar um pagamento de uma comanda", () => {
        ...
        expect(invoiceRepo.getAll()[0].getPayments().length).toBe(1)
        expect(invoiceRepo.getAll()[0].getOrders().length).toBe(4)
        expect(invoiceRepo.getAll()[0].getStatus()).toBe(INVOICE_STATUS.OPEN)
        expect(invoiceRepo.getAll()[0].getTotalPaymentsPaid()).toBe(0)
        expect(invoiceRepo.getAll()[0].getTotalOutstandingPayments()).toBe(100)
    ...
    it("deve gerar pagamento com três parcelas de 33.33 sendo que a ultima é 33.34", ()=>{
        ...
        createInvoice.execute(inputCreate)
        //console.log(invoiceRepo.getAll()[0].getPayments())
        expect(invoiceRepo.getAll()[0].getPayments().length).toBe(3)
        expect(invoiceRepo.getAll()[0].getTotalOutstandingPayments()).toBe(100)
        expect(invoiceRepo.getAll()[0].getPayments()[0].getValue()).toBe(33.33)
        expect(invoiceRepo.getAll()[0].getPayments()[2].getValue()).toBe(33.34)
    })
    ...
    it("deve gerar pagamento com seis parcelas de 16.67 sendo que a ultima é 16.65", ()=>{
        ...
        createInvoice.execute(inputCreate)
        expect(invoiceRepo.getAll()[0].getTotalOutstandingPayments()).toBe(100)
        expect(invoiceRepo.getAll()[0].getPayments()[0].getValue()).toBe(16.67)
        expect(invoiceRepo.getAll()[0].getPayments()[5].getValue()).toBe(16.65)
    })
    ...
    it("deve realizar pagamento de uma das parcelas", ()=>{
        ...
        expect(invoiceRepo.getByCode(inputInvoice.invoiceCode).getTotalPaymentsPaid()).toBe(16.67)
        expect(invoiceRepo.getByCode(inputInvoice.invoiceCode).getTotalOutstandingPayments()).toBeCloseTo(100-16.67)
        expect(invoiceRepo.getByCode(inputInvoice.invoiceCode).getPayments()[0].getStatus()).toBe(PAYMENT_STATUS.PAID)
    })
    ...
    it("deve permitir alterar o valor de uma parcela quando a soma das parcelas em aberto for menor que o total em aberto", ()=>{
        ...
        const installments = invoiceRepo.getByCode(inputAlter.invoiceCode).getPayments()
        expect(installments[0].getValue()).toBe(16)
        expect(invoiceRepo.getByCode(inputAlter.invoiceCode).getTotalOpen()).toBe(100)
        expect(invoiceRepo.getByCode(inputAlter.invoiceCode).getTotalOutstandingPayments()).toBeCloseTo(100-16.67+16)
    })
